...
class DIRControllerInformer extends DIRController
{
	var $return_url='';
...
echo '<br />'.date('Y-m-d h:m:s');
		...
		@set_time_limit(60 * 5);		
//			$model->getInfomer('Windows','http://software.informer.com/');
//			$model->getInfomer('Mac','http://mac.informer.com/');
echo '<br />'.date('Y-m-d h:m:s');
		@set_time_limit($defalutExecution);
		echo 'get category for informer.com sucessfully';
		die();
	}
	
	function getData()
	{
		require_once(dirname(__FILE__).DS.'..'.DS.'configs'.DS.'informer.php');
		
		$href	=	new href();
		$param	=	array();	
		$param['option']	=	JRequest::getVar('option');
		$param['task']		=	'informer.getData';		
		$param['live']		=	'1';
