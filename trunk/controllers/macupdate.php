...
class DIRControllerMacupdate extends DIRController
{
	var $return_url='';
	...
	function getCategory()
	{
		...
		echo 'get category for macupdate.com sucessfully';
		die();
	}
	
	function getData()
	{
		require_once(dirname(__FILE__).DS.'..'.DS.'configs'.DS.'macupdate.php');
		
		$href	=	new href();
		$param	=	array();	
		$param['option']	=	JRequest::getVar('option');
		$param['task']		=	'macupdate.getData';		
		$param['live']		=	'1';
...
//				$fp = fopen( dirname(__FILE__).DS.'..'.DS.'log'.DS.'macupdate_logfile.txt', 'a');
...
//			$query	=	'UPDATE `#__software_category_macupdate` 
...
echo '<hr /><br /> get category for macupdate.com ';
...
//		die();
		require_once(dirname(__FILE__).DS.'..'.DS.'configs'.DS.'macupdate.php');
		require_once(JPATH_COMPONENT_SITE.DS.'libraries/getfile/macupdate.php');
