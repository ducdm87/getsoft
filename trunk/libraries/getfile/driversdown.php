...
//	<a href="../Download.asp?ID=120529&amp;Url=http://down3.driversdown.com/Bios/ASUS_GPUTweak_110.zip">http://down3.driversdown.com/Bios/ASUS_GPUTweak_110.zip</a>
function getDownloadPage($body) {
    $body=str_replace("'",'"',$body); 
    $start='<a href="';
    $end='"';
    $pattern = '/'. $start .'.*?Url=([^"]+)'. $end.'/ism';

    if (preg_match_all($pattern,$body, $result)) { 
		return $result[1];
    }else {
		return false;
    }
