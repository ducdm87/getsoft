...
class DIRModelInformer extends JModel{	
	...
	function getCat()
	{
		...
		if (isset($_REQUEST['cat_id'])) {
			...
			$query = "SELECT *
				FROM `#__software_category_informer`
				WHERE id = $cat_id";
			$db->setQuery($query);
			$db->loadObject($obj);
	...
	function getListContent($link,$page =1, $cid, $secid, $catid = 0, $cat_parent, $siteID = 'if98', $SiteName ='informer.com')
	{
//		$page	=	5;		
		global $arrErr;
		$db		=	JFactory::getDBO();
		$root	=	'http://www.informer.com/';
		$href	=	new href();
	//	http://audio-multimedia.software.informer.com/
	//	http://audio-multimedia.software.informer.com/page/2/	
		if ($page > 1) {
			$link	=	$link."page/$page/";
		}
